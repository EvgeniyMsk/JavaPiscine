...
public class UsersRepositoryJdbcImpl implements UsersRepository {
    ...
    public Optional<User> findById(Long id) throws SQLException {
        ...
        if (resultSet.next())
            return Optional.of(new User(resultSet.getLong(1), resultSet.getString(2), resultSet.getString(3)));
        else
    ...
    public List<User> findAll() throws SQLException {
        ...
        while (resultSet.next())
            result.add(new User(resultSet.getLong(1), resultSet.getString(2), resultSet.getString(3)));
    ...
    public void save(User entity) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("insert into public.user(email, password) values (?, ?)");
    ...
    public void update(User entity) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement(
                "update public.user set email=?, password=? where id=?");
        preparedStatement.setString(1, entity.getEmail());
        preparedStatement.setString(2, entity.getPassword());
